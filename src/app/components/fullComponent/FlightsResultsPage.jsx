      ...
      <div className="p-5">
        <div className="flex items-start gap-4">
          <div className="flex-shrink-0">
            <AirlineLogo iata={itin.carrier} name={getAirlineName(itin.carrier)} size="md" />
          </div>
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      await fetch("/api/flight/book", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ bookingId: data.bookingId, status: result.error ? "failed" : "succeeded" }),
      });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      <motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 40 }}
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div>
            <h2 className="font-semibold text-gray-900">{step === 3 ? "Booking Confirmed" : "Book Your Flight"}</h2>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
              ...
              {passengers.map((p, i) => (
                <div key={i} className="rounded-xl border border-gray-100 overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-3 bg-gray-50">
                    ...
                    {i > 0 && <button onClick={() => removePassenger(i)} className="text-gray-400 hover:text-red-500 transition"><X size={14} /></button>}
                  </div>
          ...
          {step === 2 && (
            <Elements stripe={stripePromise}>
              <StripePaymentForm
                itin={itin} contact={contact} passengers={passengers}
                searchParams={searchParams} onSuccess={() => setStep(3)} onBack={() => setStep(1)}
              />
            </Elements>
          )}
          {step === 3 && (
            <div className="text-center py-6 space-y-5">
              ...
              <div className="bg-gray-50 rounded-2xl p-5 text-left space-y-2 text-sm">
                ...
                <div className="flex items-center justify-between text-xs text-gray-400">
                  <span>Carrier</span>
                  <span>{getAirlineName(itin.carrier)}</span>
                </div>
              </div>
