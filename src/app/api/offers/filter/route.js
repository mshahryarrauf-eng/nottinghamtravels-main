...
export async function GET(req) {
  try {
    ...
    const typeParam        = searchParams.get("type");        // ?type=Flight
    const destinationParam = searchParams.get("destination"); // ?destination=Islamabad
    ...
    // Build base query — always only active offers
    const baseQuery = { active: true };
    if (typeParam)        baseQuery.type        = { $regex: new RegExp(`^${typeParam}$`, "i") };
    if (destinationParam) baseQuery.destination = { $regex: new RegExp(`^${destinationParam}$`, "i") };

    const offers = await Offer.find(baseQuery, "dateFrom destination type");
    ...
    offers.forEach((offer) => {
      ...
      if (offer.destination) {
        ...
        destinationsMap.set(normalized.toLowerCase(), normalized);
      }
      if (offer.type) typesSet.add(normalizeString(offer.type));
    });
